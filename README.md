...
- `@nest-next/ui`: a stub React component library shared by both `web` and
  `docs` applications
- `@nest-next/eslint-config`: `eslint` configurations (includes
  `eslint-config-next` and `eslint-config-prettier`)
- `@nest-next/typescript-config`: `tsconfig.json`s used throughout the monorepo
